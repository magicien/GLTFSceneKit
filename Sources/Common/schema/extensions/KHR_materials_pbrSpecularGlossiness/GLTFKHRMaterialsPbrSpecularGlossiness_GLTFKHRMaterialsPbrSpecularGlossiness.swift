    ...
    func didLoad(by object: Any, unarchiver: GLTFUnarchiver) {
        ...
        if let specularGlossinesTexture = data.specularGlossinessTexture {
            ...
            material.shaderModifiers = [
                .surface: try! String(contentsOf: Bundle.module.url(forResource: "GLTFShaderModifierSurface_pbrSpecularGlossiness_texture_doubleSidedWorkaround", withExtension: "shader")!, encoding: String.Encoding.utf8)
            ]
        } else {
            ...
            material.shaderModifiers = [
                .surface: try! String(contentsOf: Bundle.module.url(forResource: "GLTFShaderModifierSurface_pbrSpecularGlossiness", withExtension: "shader")!, encoding: String.Encoding.utf8)
            ]
            
            #if SEEMS_TO_HAVE_DOUBLESIDED_BUG
                if material.isDoubleSided {
                    material.shaderModifiers = [
                        .surface: try! String(contentsOf: Bundle.module.url(forResource: "GLTFShaderModifierSurface_pbrSpecularGlossiness_doubleSidedWorkaround", withExtension: "shader")!, encoding: String.Encoding.utf8)
                    ]
                }
            #endif
        }
